...
/**
 * Clase para inicializar datos de prueba en la base de datos si está vacía.
...
public class DataInitializer {
    ...
    /**
     * Carga los productos de prueba al iniciar la aplicación si la base de datos está vacía.
     */
    @Bean
    public CommandLineRunner loadData() {
        return args -> {
            if (productService.getAllProducts().isEmpty()) {
                logger.info("Base de datos de productos vacía. Inicializando datos de prueba...");
                List<Product> initialProducts = createInitialProducts();
                productService.importProducts(initialProducts);
                logger.info("Se han insertado {} productos de prueba.", initialProducts.size());
            } else {
                logger.info("La base de datos de productos ya contiene datos. No se requiere inicialización.");
            }
        };
    }

    /**
     * Crea y devuelve una lista de 20 productos de prueba.
